...
- **Provider:** Claude (Anthropic SDK), GLM (OpenAI-compat), 9Router (proxy lokal), GLM Vision (glm-4.6v, menerima gambar → `create_character` stilasi low-poly) — satu antarmuka `LLMAdapter`. API key disimpan di browser, panggilan langsung client → provider.
