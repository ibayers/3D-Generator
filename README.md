...
- **Provider:** Claude (Anthropic SDK), GLM (OpenAI-compat), 9Router (proxy lokal), GLM Vision (glm-5.3-flash multimodal via langganan Coding Plan, menerima gambar → `create_character` stilasi low-poly) — satu antarmuka `LLMAdapter`. API key disimpan di browser; panggilan z.ai melewati proxy rewrite Next (tanpa CORS).
